...
public class Modulo extends Aritmeticas {
    ...
    public Object Ejecutar(Entorno e) {
        ...
        if (aux.tp == TipoExp.Tipos.VECTOR) {
            ...
            if (top1.isMatrix() && top2.isMatrix()) {
                return ModuloMatriz_Matriz((Matrix) valor1, (Matrix) valor2, e);
            } //con un vector
            else if (top1.isVector() || top2.isVector()) //con un primitivo
            {
                return top1.isMatrix() ? ModuloMatriz_Vector((Matrix) valor1, (EstructuraLineal) valor2, e) : ModuloMatriz_Vector((Matrix) valor2, (EstructuraLineal) valor1, e);
            } else if (top1.isPrimitive(e) || top2.isPrimitive(e)) {
                return top1.isMatrix() ? ModuloMatriz_Primitivo((Matrix) valor1, valor2, e) : ModuloMatriz_Primitivo((Matrix) valor2, valor1, e);
            } else {
                return new Errores(Errores.TipoError.SEMANTICO, "La matriz no puede hacer modulo con ese tipo de objeto " + top1.toString() + top2.toString(), linea, columna);
            }
        }
        switch (aux.tp) {
            case NULO:
    ...
    private Object ModuloMatriz_Primitivo(Matrix a, Object b, Entorno e) {
        ...
        for (int i = 0; i < a.getColumna(); i++) {
            ...
            for (int j = 0; j < a.getFila(); j++) {
                ...
                Object res = new Modulo((Expresion) va.getDimensiones().get(0), (Expresion) lb, op, linea, columna).getValor(e);
    ...
    private Object ModuloMatriz_Vector(Matrix a, EstructuraLineal b, Entorno e) {
    ...
    private Object ModuloMatriz_Matriz(Matrix a, Matrix b, Entorno e) {
        ...
        if ((a.getColumna() == b.getColumna()) && (a.getFila() == b.getFila())) {
            ...
            for (int i = 0; i < a.getColumna(); i++) {
                ...
                for (int j = 0; j < a.getFila(); j++) {
                    ...
                    Object res = new Modulo((Expresion) va.getDimensiones().get(0), (Expresion) vb.getDimensiones().get(0), op, linea, columna).getValor(e);
                    if (res instanceof Errores) {
                        return res;
                    }
                    TipoExp origen = Globales.VarGlobales.getInstance().obtenerTipo(res, e);
            ...
            return new Errores(Errores.TipoError.SEMANTICO, "No se puede hacer modulo con matrices que no sean del mismo tamaño", linea, columna);
        }
    }
