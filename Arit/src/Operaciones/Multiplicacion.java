...
public class Multiplicacion extends Aritmeticas {
    ...
    public Object ejecutar(Entorno e) {
        ...
        if (aux.tp == TipoExp.Tipos.VECTOR) {
            ...
            if (top1.isVector() && top2.isVector()) {
                return MulVectoresVectores((EstructuraLineal) valor1, (EstructuraLineal) valor2, e);
            }
            return top1.isVector() ? MulVectores((EstructuraLineal) valor1, top2, valor2, e, true) : MulVectores((EstructuraLineal) valor2, top1, valor1, e, false);
        } else if (aux.isMatrix()) {
            //suma de matriz con matrices
            if (top1.isMatrix() && top2.isMatrix()) {
                return MultiMatriz_Matriz((Matrix) valor1, (Matrix) valor2, e);
            } //con un vector
            else if (top1.isVector() || top2.isVector()) //con un primitivo
            {
                return top1.isMatrix() ? MultiMatriz_Vector((Matrix) valor1, (EstructuraLineal) valor2, e) : MultiMatriz_Vector((Matrix) valor2, (EstructuraLineal) valor1, e);
            } else if (top1.isPrimitive(e) || top2.isPrimitive(e)) {
                return top1.isMatrix() ? MultiMatriz_Primitivo((Matrix) valor1, valor2, e) : MultiMatriz_Primitivo((Matrix) valor2, valor1, e);
            } else {
                return new Errores(Errores.TipoError.SEMANTICO, "La matriz no puede multiplicar con ese tipo de objeto " + top1.toString() + top2.toString(), linea, columna);
            }
        }
        switch (aux.tp) {
            case NULO:
    ...
    private Object MultiMatriz_Primitivo(Matrix a, Object b, Entorno e) {
        ...
        for (int i = 0; i < a.getColumna(); i++) {
            ...
            for (int j = 0; j < a.getFila(); j++) {
                ...
                Object res = new Multiplicacion((Expresion) va.getDimensiones().get(0), (Expresion) lb, op, linea, columna).getValor(e);
    ...
    private Object MultiMatriz_Vector(Matrix a, EstructuraLineal b, Entorno e) {
    ...
    private Object MultiMatriz_Matriz(Matrix a, Matrix b, Entorno e) {
        ...
        if ((a.getColumna() == b.getColumna()) && (a.getFila() == b.getFila())) {
            ...
            for (int i = 0; i < a.getColumna(); i++) {
                ...
                for (int j = 0; j < a.getFila(); j++) {
                    ...
                    Object res = new Multiplicacion((Expresion) va.getDimensiones().get(0), (Expresion) vb.getDimensiones().get(0), op, linea, columna).getValor(e);
                    if (res instanceof Errores) {
                        return res;
                    }
                    TipoExp origen = Globales.VarGlobales.getInstance().obtenerTipo(res, e);
            ...
            return new Errores(Errores.TipoError.SEMANTICO, "No se puede multiplicar matrices que no sean del mismo tamaño", linea, columna);
        }
    }

    private Object MulVectores(EstructuraLineal v, TipoExp tipoexp, Object valorsumando, Entorno e, boolean primero) {
    ...
    private Object MulVectoresVectores(EstructuraLineal v1, EstructuraLineal v2, Entorno e) {
