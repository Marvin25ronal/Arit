 ...
 * @author marvi
 */
public class Round implements Expresion {

    LinkedList<Expresion> exp;
    LinkedList<Expresion> dimensiones;
    int linea, columna;
    
    public Round(LinkedList<Expresion> exp, LinkedList<Expresion> dimensiones, int linea, int columna) {
        this.exp = exp;
        ...
        if (exp.size() == 1) {
            ...
            if (t.isVector()) {
                ...
                if (v.getTiposecundario().esNumero()) {
                    ...
                    int casteado = (int) Math.round(valor);
                    EstructuraLineal nueva = new EstructuraLineal(new TipoExp(TipoExp.Tipos.INTEGER), casteado, linea, columna);
